...
const API_BASE = "http://192.168.0.101:8010"
...
export const CameraControl: React.FC<CameraControlProps> = ({ activeButtons, onButtonPress }) => {
  ...
  const handlePress = (key: string, active: boolean) => {
    ...
    if (key === "a") fetch("http://192.168.0.101:8010/camera/start", { method: "POST" });
    if (key === "b") fetch("http://192.168.0.101:8010/camera/stop", { method: "POST" });
    if (key === "x") fetch("http://192.168.0.101:8010/camera/video", { method: "POST" });
    if (key === "y") fetch("http://192.168.0.101:8010/camera/photo", { method: "POST" });
    if (key === "c") fetch("http://192.168.0.101:8010/camera/transfer", { method: "POST" });
    if (key === "d") fetch("http://192.168.0.101:8010/camera/setup", { method: "POST" });
    if (key === "e") fetch("http://192.168.0.101:8010/camera/shutdown", { method: "POST" });
  };
